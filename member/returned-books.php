...
if($rownum){
	...
	$fetchbook=mysql_fetch_array($sqlbook);
	   header("location:returned-books?search=".base64_encode($fetchbook['id']));
}else{
...
'shelf'=>$fetch['shelf']
			...
			$sqlbk=mysql_query("SELECT * FROM returnedbooks WHERE bookid='".$fetch['id']."'");
...
$sql=nacoss::$connect->query("SELECT * FROM book ORDER BY id DESC limit 0, 15");
if($sql->rowCount() > 0){
...
'shelf'=>$fetch['shelf']
		...
		$sqlbk=mysql_query("SELECT * FROM returnedbooks WHERE bookid='".$fetch['id']."' AND memberid='".$_SESSION['nacoss_user']."'");
		$rowbk=mysql_num_rows($sqlbk);
